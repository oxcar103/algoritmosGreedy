...
\begin{algorithm}[H]
	\begin{algorithmic}[1]
		...
		\FORALL{\texttt{moneda} en \texttt{monedas}}
			...
			\STATE{\texttt{precio} $\gets$ \texttt{precio} $-$ \texttt{cantidad} \* \texttt{moneda}}
...
\subsection{Contraejemplo: Sellos} % Contraejemplo?
...
\section{Red de comunicaciones}
  \subsection{Algoritmo}
    ...
    $V = \left\{ [a,b] |\ a,b \in C \right\}$, donde la arista conectando los nodos $a$ y $b$ tiene peso 
  ...
  \subsection{Triangulación de Delaunay}
    Para reducir la carga del algoritmo, podemos reducir el grafo sobre que que buscamos el árbol generador minimal.
    ...
    \begin{proof}
      ...
      Consideramos el círculo que tiene como diámetkruro $\overline{pq}$, si hubiera otro punto $r$ en este círculo, tendríamos:
      \begin{equation}
...
($s_i$ y $f_i$, $i\in\{1\ldots n\}$) y queremos encontrar el mínimo número de aulas necesarias
