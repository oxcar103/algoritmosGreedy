...
Si conocemos de antemano el conjunto de solicitudes que deberemos atender $p_1, p_2, \ldots, p_n$ peticiones con el menor número de fallos en caché
  \subsection{Algoritmo}
  La técnica que se seguirá es ir escogiendo el elemento que se va a sacar de caché de tal manera que sea el elemento que se usará más tarde(\textbf{Farthest-In-Future}) cada vez que se produzca un fallo en caché:
\begin{algorithm}[H]
	\begin{algorithmic}[1]
		\REQUIRE \ \\
     	...
     	\STATE{fallos = 0}\\
     	\STATE{$p_{reducido}$ = eliminarDuplicadosConsecutivos($p$)\\
     	Esto se realiza para agilizar el recorrido del array, ya que si están consecutivos está asegurado que, a partir de la segunda ocurrencia, estarán en caché.}
     	%No estoy muy seguro de este STATE
     	\STATE{caché = eliminarDuplicados($p_{reducidos}$)\\
     	     	Inicializamos la memoria con los primeros k datos no repetidos}
	  \FORALL{\texttt{petición}$\in$\texttt{$p$}}
	   \IF{\texttt{petición $\notin$ caché}}
		\STATE{Buscamos(en $p_{reducido}$) el elemento que está en caché que más tardará en volver a ser necesitado}
