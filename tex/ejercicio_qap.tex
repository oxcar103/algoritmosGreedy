...
\begin{algorithm}[H]
	\begin{algorithmic}[1]
	  ...
	  \FORALL{\texttt{i} $\in \{1\ldots s\}$}
	    ...
	    \STATE{\texttt{max\_w\_index=j : w(k,j)=}$\max\{w(j,k)\texttt{!fab\_asignada[j]}\}$}
	    \STATE{\texttt{min\_d\_index=j : d(actual.permutacion[k],j)=}
	     \qquad$\min\{d(j,p(k))\texttt{!loc\_asignada[j]}\}$}
...
en este documento. En ese caso, la eficiencia de OPT-2 sería $\mathcal{O}(\max(n^2,\texttt{eficiencia(greedy)}))$
